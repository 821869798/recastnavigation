...
#endif
			// 获取这个脚本所在的程序集的名称
			string assemblyName = typeof(EditorResourceHelper).Assembly.GetName().Name;

			// 获取程序集的Assembly Definition文件路径
			string debugPath = UnityEditor.Compilation.CompilationPipeline.GetAssemblyDefinitionFilePathFromAssemblyName(assemblyName);

			editorAssets = Path.Combine(Path.GetDirectoryName(Path.GetDirectoryName(debugPath)), "Editor", "EditorAssets").Replace('\\', '/');
