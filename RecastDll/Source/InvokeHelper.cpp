...
NavMeshContex* RecastLoad_Old(int32_t id, const char* buffer, int32_t n)
{
	return NavMesh::GetInstace()->New(id, buffer, n);
}

NavMeshContex* RecastGet_Old(int32_t id)
{
	return NavMesh::GetInstace()->Get(id);
}

void RecastClear_Old()
{
	NavMesh::GetInstace()->Clear();
}

int32_t RecastFind_Old(NavMeshContex* navMeshContex, float* extents, float* startPos, float* endPos, float* straightPath)
{
...
int32_t RecastFindNearestPoint_Old(NavMeshContex* navMeshContex, float* extents, float* startPos, float* nearestPos)
{
	if (navMeshContex == nullptr)
	{
		return -1;
...
int32_t RecastFindRandomPoint_Old(NavMeshContex* navMeshContex, float* pos)
{
	if (navMeshContex == nullptr)
	{
		return -1;
...
int32_t RecastFindRandomPointAroundCircle_Old(NavMeshContex* navMeshContex, float* extents, const float* centerPos, const float maxRadius, float* pos)
{
	if (navMeshContex == nullptr)
	{
		return -1;
